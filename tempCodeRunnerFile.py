    ...
    if request.method == "POST" and not error_msg:
        try:
            ...
            row.update({
                ...
                "JUMLAH_MINYAK": get_amount("JUMLAH_MINYAK"),
                "JUMLAH_SURFAKTAN": get_amount("JUMLAH_SURFAKTAN"),
                "JUMLAH_LOGAM": " | ".join([
                    request.form.get("JUMLAH_LOGAM_1", get_amount("JUMLAH_LOGAM")),
                    request.form.get("JUMLAH_LOGAM_2", "")
                ]).strip(" |"),
                "JUMLAH_KO_SURFAKTAN": get_amount("JUMLAH_KO_SURFAKTAN"),
                ...
                # Metal special case: split or single
                "JUMLAH LOGAM (g)": " | ".join([
                    request.form.get("JUMLAH_LOGAM_1", get_amount("JUMLAH_LOGAM")),
                    request.form.get("JUMLAH_LOGAM_2", "")
                ]).strip(" |"),
            ...
            for key, val in form_data.items():
                translated_val = val
                for cat, mapping in display_values.items():
                    if val in mapping:
                        translated_val = mapping[val]
                        break
                    if " | " in val:
                        parts = [mapping.get(p.strip(), p.strip()) for p in val.split("|")]
                        translated_val = " | ".join(parts)
                translated_form_data[key] = translated_val
            ...
            if "|" in form_data.get("LOGAM", ""):
                translated_form_data["JUMLAH_LOGAM"] = " | ".join(filter(None, [
                    request.form.get("JUMLAH_LOGAM_1", ""),
                    request.form.get("JUMLAH_LOGAM_2", "")
                ]))
            else:
                translated_form_data["JUMLAH_LOGAM"] = get_amount("JUMLAH_LOGAM")
